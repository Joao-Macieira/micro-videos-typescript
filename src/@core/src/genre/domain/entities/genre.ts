...
import { AggregateRoot, EntityValidationError, UniqueEntityId } from "#seedwork/domain";
import { GenreValidatorFactory } from "../validator";
...
export class Genre extends AggregateRoot<GenreId, GenreProperties, GenrePropsJson> {
  ...
  static create(props: GenreCreateCommand, id?: GenreId) {
    ...
    props.categories_id.forEach(categoryId => {
      categories_id.set(
        ...
        categoryId instanceof CategoryId ? categoryId : new CategoryId(categoryId)
      );
    });
  ...
  toJSON(): GenrePropsJson {
    return {
      id: this.id,
      name: this.name,
      categories_id: Array.from(this.props.categories_id.values()).map(categoryId => categoryId.value),
