...
import { CastMember, CastMemberRepository, Types } from "../../../../domain";
...
describe("ListCastMembersUseCase Unit Tests", () => {
  ...
  it("should search sorted by created_at when input param is empty", async () => {
    ...
    expect(output).toStrictEqual({
      items: [...items].reverse().map(CastMemberToOutputMapper.toOutput),
      total: 2,
      current_page: 1,
      per_page: 15,
      last_page: 1,
  ...
  it("should search applying paginate and filter by type", async () => {
    ...
    const arrange = [
      ...
      },
      {
        ...
        output: {
          items: [castMembers[2]].map(CastMemberToOutputMapper.toOutput),
          total: 3,
          current_page: 2,
          per_page: 2,
          last_page: 2,
