...
class StubEntity extends Entity<UniqueEntityId ,StubEntitiesProps> {
  constructor(props: StubEntitiesProps, entityId?: UniqueEntityId) {
    super(props, entityId ?? new UniqueEntityId());
  }

  toJSON(): Required<{ id: string; } & StubEntitiesProps> {
    return {
      id: this.id,
...
describe("InMemorySearchableRepository Unit tests", () => {
  ...
  describe('search method', () => {
    ...
    it('should search using filter, sort and paginate', async () => {
      ...
      const arrange = [
        {
          params: new SearchParams({ page: 1, per_page: 2, sort: 'name', filter: 'TEST' }), // Letra maiúscula vem antes de acordo com a tabel ASCII
          result: new SearchResult({
        ...
        {
          params: new SearchParams({ page: 1, per_page: 2, sort: 'name', sort_dir: 'desc', filter: 'TEST' }),
          result: new SearchResult({
        ...
        {
          params: new SearchParams({ page: 1, per_page: 2, sort: 'name', sort_dir: 'desc', filter: 'a' }),
          result: new SearchResult({
            items: [items[1]],
            total: 1,
            current_page: 1,
